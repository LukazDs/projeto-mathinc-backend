...
const userSignInSchema = joi.object<IUserCreationLogin>({
...
const userSignUpSchema = joi.object<IUserCreationReg>({
  name: joi.string().email().required().label("Informe um nome válido!"),
  email: joi.string().email().required().label("Informe um email válido!"),
